...
static void handle_cmd(struct android_app *app, int32_t command) {
    ...
    switch (command) {
        case APP_CMD_INIT_WINDOW:
            ...
            render_set_window(app->window);   /* Vulkan presents through the surface */
            if (!render_select(app_assets, screen_w, screen_h, NULL)) { init_done = 0; return; }
            app_log("renderer: %s", render_backend_name());
            /* Sounds live in the same assets folder (sounds/...), output via AudioTrack. */
            audio_init(app_assets);
            audio_resume();
            init_done = 1;
            app_active = 0;
            app_clear_error();
            /* game_init() builds the world itself: do not generate all chunks twice. */
