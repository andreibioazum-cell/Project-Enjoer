...
int geometrium_face_exposed(int b,int n) {
    return b!=BLOCK_AIR && (n==BLOCK_AIR || (n==BLOCK_WATER && b!=BLOCK_WATER));
}
static void emit(GeometriumChunk *c,const int origin[3],int u,int v,int face,int block,const unsigned char light[4]) {
    if (c->count==c->capacity) {
        int capacity=c->capacity ? c->capacity*2 : 256;
        GeometriumQuad *p=realloc(c->quads,(size_t)capacity*sizeof(*p));
    ...
    c->quads[c->count++]=(GeometriumQuad){origin[0],origin[1],origin[2],u,v,face,block,{light[0],light[1],light[2],light[3]}};
...
static void half_face(GeometriumChunk *c,int sx,int sy,int sz,int face,int block) {
    int n=geometrium_world_cell(c->cx*CHUNK_SIZE*2+sx+normal[face][0],sy+normal[face][1],
                         c->cz*CHUNK_SIZE*2+sz+normal[face][2]);
    if (!geometrium_face_exposed(block,n)) return;
    int p[3]={sx,sy,sz};
    for (int axis=0;axis<3;axis++) if (normal[face][axis]>0) p[axis]++;
    unsigned char light[4];geometrium_light_face(c,p,1,1,face,light);
    emit(c,p,1,1,face,block,light);
}
static void split_boundary(GeometriumChunk *c,const int p[3],int a,int ua,int va,int face,int b) {
...
void geometrium_chunk_mesh(GeometriumChunk *c) {
        ...
        for (int slice=0;slice<dims[a];slice++) {
            ...
            for (int v=0;v<height;v++) for (int u=0;u<width;u++) {
                ...
                int n=inside ? blocks[at(neighbor[0],neighbor[1],neighbor[2])] :
                    geometrium_world_uniform(ox+neighbor[0],neighbor[1],oz+neighbor[2]);
                if (n==BLOCK_PARTIAL) split_boundary(c,p,a,ua,va,face,b);
                else if (geometrium_face_exposed(b,n)) {
                    int origin[3]={p[0]*2,p[1]*2,p[2]*2};
                    origin[a]+=normal[face][a]>0 ? 2 : 0;
                    unsigned char light[4];geometrium_light_face(c,origin,2,2,face,light);
    ...
    c->ready=1;c->dirty=0;c->serial++; /* hardware backends re-upload on a new serial */
}
