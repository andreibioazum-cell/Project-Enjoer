...
int sw_geometrium3d_begin(Buffer *b, int sc, float cx, float cy, float cz, float yaw, float pitch, float fov_deg) {
...
void sw_geometrium3d_sky(uint32_t top, uint32_t bot) {
    if (!dst) return;
...
void sw_geometrium3d_viewmodel(int enabled) {viewmodel=enabled!=0;}
...
void sw_geometrium3d_polygon(const GeometriumVertex *w, int n, float nx, float ny, float nz,
    ...
    float lo=LIGHT_LEVELS,hi=0;
    unsigned clip=0,all=63;
    for (int i=0;i<n;i++) {
        to_view(w[i].x,w[i].y,w[i].z,&in[i]);
        in[i].u=w[i].u;in[i].v=w[i].v;
        float shade=geometrium_vertex_shade(nx,ny,nz,light ? light[i] : 255);
        in[i].light=shade*(LIGHT_LEVELS-1);
        if (!isfinite(shade)) in[i].light=0;
