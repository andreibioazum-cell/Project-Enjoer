...
int geometrium_world_chunk_view(int cx,int cz,GeometriumChunkView *out) {
    if (!out) return 0;
    GeometriumChunk *c=slot(cx,cz);
    if (!matches(c,cx,cz)) return 0;
    out->cx=cx;out->cz=cz;out->ready=c->ready;out->quads=c->quads;
    out->count=c->count;out->water_count=c->water_count;out->serial=c->serial;
    out->min_y=c->min_y*.5f;out->max_y=c->max_y*.5f;
    return 1;
}
void geometrium_world_center(int *cx,int *cz) { if(cx)*cx=center_x;if(cz)*cz=center_z; }
void geometrium_world_draw(void) {
    /* A hardware backend keeps its own per-chunk buffers, so it walks the same
     * rings itself instead of submitting one quad at a time. */
    if (render_backend && render_backend->chunks) { render_backend->chunks(center_x,center_z,WORLD_RADIUS); return; }
    draw_pass(0); draw_pass(1);
}
