...
int geometrium_raycast(float x,float y,float z,float dx,float dy,float dz,float reach,GeometriumHit *hit) {
    ...
    for (int n=0;n<256 && distance<=reach;n++) {
        int b=geometrium_world_cell(cell[0],cell[1],cell[2]);
...
int geometrium_action_apply(int action,const GeometriumHit *h) {
    ...
    if (!geometrium_cell_solid(h->x,h->y,h->z)) return 0;
    if (action==ACTION_BREAK) {
        if (h->y<2) return 0;
        if (!geometrium_world_set(h->x,h->y,h->z,BLOCK_AIR)) return 0;
    ...
    if (geometrium_cell_solid(x,y,z) || geometrium_player_overlaps(x,y,z)) return 0;
    if (!geometrium_world_set(x,y,z,slots[selected])) return 0;
...
void geometrium_target_draw(void) {
    ...
    for (int axis=0;axis<3;axis++) for (int a=0;a<2;a++) for (int b=0;b<2;b++) {
        ...
        geometrium3d_segment(v[0],v[1],v[2],end[0],end[1],end[2],0xffffffffu);
    }
}
