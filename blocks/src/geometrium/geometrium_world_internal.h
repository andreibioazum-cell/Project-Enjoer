...
/* Snapshot handed to a hardware backend: the quads of one chunk mesh plus the
 * serial that changes whenever the mesh is rebuilt. */
typedef struct {
    int cx,cz,ready,count,water_count;
    unsigned serial;
    const GeometriumQuad *quads;
    float min_y,max_y;
} GeometriumChunkView;
typedef struct {
    int cx,cz,valid,ready,dirty,min_y,max_y;
    unsigned serial; /* mesh revision, bumped by geometrium_chunk_mesh() */
...
/* Ring-ordered chunk access for backends that draw whole chunks on the GPU. */
int geometrium_world_chunk_view(int cx,int cz,GeometriumChunkView *out);
void geometrium_world_center(int *cx,int *cz);
