...
int geometrium_materials_load(AAssetManager *assets) {
    ...
    for (int t=0;t<MATERIALS;t++) {
        ...
        build_mips(m);
        m->layer=t; /* texture array index used by the Vulkan backend */
    }
...
int geometrium_material_layer(int block,int face) { return initialized ? texture(block,face) : 0; }
int geometrium_material_count(void) { return initialized ? MATERIALS : 0; }
const Image *geometrium_material_image(int layer) {
    return initialized && layer>=0 && layer<MATERIALS ? &materials[layer].image : NULL;
}
int geometrium_material_alpha(int layer) {
    return initialized && layer>=0 && layer<MATERIALS ? materials[layer].alpha : 0;
}
int geometrium_material_water(int layer) { return layer==WATER; }
int geometrium_material_layer_of(const Image *image) {
    if (!image || !initialized) return 0;
    for (int i=0;i<MATERIALS;i++) if (&materials[i].image==image) return materials[i].layer;
    return 0;
}
