...
The game draws through one table (`src/render.h`), and two backends implement
it:

- **Software rasterizer** (`src/geometrium/geometrium_render.c`) — plain C, no
  driver, always available. It is the reference the regression tests measure.
- **Vulkan** (`src/vk/`) — one vertex/index buffer per chunk mesh, the block
  textures in a 2D array with mipmaps, a single HUD batch and an upscale pass.
  It never links `libvulkan`: the loader is opened with `dlopen()` when the
  game starts, so a machine without a driver still builds and still plays.

Both share the same camera, clipping planes, radial fog, face shading and
material table (`src/geometrium/geometrium_material.c`), so a software frame and
a Vulkan frame show the same world; only the filling differs. Water is drawn in
a second transparent pass in both backends.

```sh
VULKAN_HEADERS_DIR=/path/to/Vulkan-Headers/include tools/preview/build.sh
./preview --render auto        # Vulkan when a device exists, else software
./preview --render vulkan      # try Vulkan, log the reason and fall back
./preview --render vulkan!     # strict: refuse to fall back (exit 1)
./preview --render software    # force the rasterizer
```

`tools/vulkan/headers.sh` fetches the Khronos headers for a desktop build; the
Android NDK already ships them, and the APK build always compiles the Vulkan
backend in and prefers it at runtime.
