...
int main(int argc, char **argv) {
    ...
    const char *render = NULL;   /* auto: Vulkan when available, else software */
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--render") && i + 1 < argc) render = argv[++i];
        else if (!strcmp(argv[i], "--help") || !strcmp(argv[i], "-h")) {
            fprintf(stderr, "usage: preview [--port N] [--w N] [--h N] [--assets DIR] [--storage DIR]\n"
                            "               [--render auto|vulkan|vulkan!|software]\n");
            return 0;
        }
    }
    if (render && strcmp(render, "auto") && strcmp(render, "vulkan") && strcmp(render, "vk") &&
        strcmp(render, "vulkan!") && strcmp(render, "software") && strcmp(render, "sw")) {
        fprintf(stderr, "Unknown renderer '%s' (auto|vulkan|vulkan!|software)\n", render);
        return 1;
    }
    ...
    if (!render_select(am, w, h, render)) { fprintf(stderr, "graphics init failed\n"); return 1; }
