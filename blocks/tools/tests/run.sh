...
CORE="src/core/log.c src/core/state.c src/core/assets.c src/render_state.c tools/preview/host_compat.c"
# The drawing entry points are backend-agnostic: src/render.c forwards them to a
# backend table, so the tests always link the software one (render_sw.c).
SWRENDER="src/render.c src/render_sw.c src/graphics/gfx_font.c src/graphics/gfx_frame.c src/graphics/gfx_draw.c src/graphics/gfx_text.c"
FONT="src/graphics/ttf/ttf_font.c src/graphics/ttf/ttf_outline.c"
...
$CC $FLAGS $CORE $SWRENDER $FONT src/graphics/image.c src/geometrium/geometrium_render.c src/geometrium/geometrium_shapes.c src/geometrium/geometrium_material.c tools/tests/render.c -lm -o "$DIR/render"
"$DIR/render"
$CC $FLAGS $CORE $FONT src/geometrium/geometrium_hand.c tools/tests/hand.c -lm -o "$DIR/hand"
"$DIR/hand"
...
$CC $FLAGS $CORE $WORLD $ACTORS src/geometrium/geometrium_perf.c src/geometrium/geometrium_game.c src/geometrium/geometrium_scene.c src/geometrium/geometrium_hud.c src/geometrium/geometrium_hand.c \
    src/geometrium/geometrium_render.c src/geometrium/geometrium_shapes.c src/geometrium/geometrium_material.c src/graphics/image.c $SWRENDER $FONT \
