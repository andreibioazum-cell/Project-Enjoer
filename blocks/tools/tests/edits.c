...
void geometrium3d_surface(int x,int y,int z,int u,int v,int f,int b,const unsigned char s[4]) {(void)x;(void)y;(void)z;(void)u;(void)v;(void)f;(void)b;(void)s;}
void geometrium3d_segment(float x,float y,float z,float a,float b,float c,uint32_t color) {(void)x;(void)y;(void)z;(void)a;(void)b;(void)c;(void)color;}
/* Rasterizer stubs: geometrium_hand.c links here because of the shared actions. */
void geometrium3d_polygon(const GeometriumVertex *w,int n,float nx,float ny,float nz,uint32_t color,GeometriumMaterial *material,const unsigned char light[4])
{(void)w;(void)n;(void)nx;(void)ny;(void)nz;(void)color;(void)material;(void)light;}
int geometrium3d_project(float x,float y,float z,float *sx,float *sy) {(void)x;(void)y;(void)z;if(sx)*sx=0;if(sy)*sy=0;return 1;}
void geometrium3d_viewmodel(int enabled) {(void)enabled;}
int geometrium3d_face_visible(int face,float plane) {(void)face;(void)plane;return 1;}
void geometrium3d_depth_clear(float x0,float y0,float x1,float y1) {(void)x0;(void)y0;(void)x1;(void)y1;}
GeometriumMaterial *geometrium_material(int block,int face) {(void)block;(void)face;return NULL;}
static void fresh(void) {app_set_storage(NULL);geometrium_cancel_input();geometrium_world_build(GEOMETRIUM_WORLD_SEED);geometrium_player_spawn();geometrium_input_layout();}
...
static void test_eight_parts(void) {
    fresh();
    for(int i=0;i<8;i++)CHECK(geometrium_world_cell(16+(i&1),24+((i>>1)&1),16+(i>>2))==BLOCK_GRASS);
    CHECK(geometrium_world_set(17,25,16,BLOCK_AIR));int empty=0;
    for(int i=0;i<8;i++)empty+=geometrium_world_cell(16+(i&1),24+((i>>1)&1),16+(i>>2))==BLOCK_AIR;
    CHECK(empty==1 && geometrium_world_uniform(8,12,8)==BLOCK_PARTIAL && geometrium_edits_count()==1);
    CHECK(geometrium_world_set(17,25,16,BLOCK_GRASS));CHECK(geometrium_world_uniform(8,12,8)==BLOCK_GRASS);
    CHECK(!geometrium_world_set(17,25,16,BLOCK_GRASS));
    CHECK(!geometrium_world_set(17,1,16,BLOCK_AIR));CHECK(!geometrium_world_set(17,128,16,BLOCK_STONE));
    CHECK(!geometrium_world_set(17,25,16,BLOCK_COUNT));
...
static void test_ray_and_build(void) {
    ...
    for(int i=0;i<8;i++)CHECK(geometrium_world_cell(16+(i&1),24+((i>>1)&1),16+(i>>2))==BLOCK_AIR);
    for(int layer=0;layer<2;layer++)for(int z=0;z<2;z++)for(int x=0;x<2;x++) {
        CHECK(geometrium_raycast(8.25f+x*.5f,16.75f,8.25f+z*.5f,0,-1,0,6,&h));
        CHECK(h.y==23+layer);CHECK(geometrium_action_apply(ACTION_PLACE,&h));
    }
    CHECK(geometrium_world_uniform(8,12,8)==BLOCK_GRASS);
    ...
    CHECK(geometrium_world_set(-1,90,-1,BLOCK_STONE));
    ...
    CHECK(geometrium_world_set(17,27,18,BLOCK_STONE));h=(GeometriumHit){17,27,18,0,0,-1,BLOCK_STONE,1};
    CHECK(!geometrium_action_apply(ACTION_PLACE,&h)); /* would overlap the player */
    geometrium_select(2);h.nz=1;CHECK(geometrium_action_apply(ACTION_PLACE,&h));CHECK(geometrium_world_cell(17,27,19)==BLOCK_STONE);
    ...
    CHECK(geometrium_world_set(4,2,4,BLOCK_AIR));CHECK(geometrium_action_apply(ACTION_PLACE,&h)); /* build on bedrock */
...
static uint32_t nearby(void) {
    ...
    for(int y=10;y<32;y++)for(int z=12;z<24;z++)for(int x=12;x<24;x++)h=(h^(uint32_t)geometrium_world_cell(x,y,z))*16777619u;
    return h;
}
static void test_edit_controls(void) {
    ...
    geometrium_input_touch(x,y,0,52);geometrium_input_touch(x,y,1,52);geometrium_actions_update(.016f);CHECK(geometrium_world_cell(h.x,h.y,h.z)==BLOCK_AIR);
    geometrium_input_touch(x,y,0,53);geometrium_key_state("break",1);geometrium_actions_update(.016f);
    geometrium_input_touch(x,y,4,53);CHECK(geometrium_target(&h));geometrium_actions_update(.25f);CHECK(geometrium_world_cell(h.x,h.y,h.z)==BLOCK_AIR);
    ...
    CHECK(geometrium_target(&h));geometrium_actions_update(.25f);CHECK(geometrium_world_cell(h.x,h.y,h.z)==BLOCK_AIR);
...
static void no_overlap(void) {
    ...
    for(int y=(int)floorf((py+.001f)*2);y<=(int)floorf((py+GEOMETRIUM_PLAYER_HEIGHT-.001f)*2);y++)
        for(int z=(int)floorf((pz-GEOMETRIUM_PLAYER_RADIUS+.001f)*2);z<=(int)floorf((pz+GEOMETRIUM_PLAYER_RADIUS-.001f)*2);z++)
            for(int x=(int)floorf((px-GEOMETRIUM_PLAYER_RADIUS+.001f)*2);x<=(int)floorf((px+GEOMETRIUM_PLAYER_RADIUS-.001f)*2);x++)CHECK(!geometrium_cell_solid(x,y,z));
}
static void test_half_collisions(void) {
    fresh();for(int z=16;z<18;z++)for(int x=16;x<18;x++)CHECK(geometrium_world_set(x,25,z,BLOCK_AIR));
    ...
    fresh();aim(0,0);
    for(int sy=26;sy<31;sy++)for(int sx=15;sx<20;sx++)if(sx!=17)CHECK(geometrium_world_set(sx,sy,18,BLOCK_STONE));
    ...
    for(int sy=26;sy<30;sy++)CHECK(geometrium_world_set(16,sy,18,BLOCK_AIR));
...
static void warm(float x,float z) {
    geometrium_world_update(x,z);for(int i=0;i<500 && geometrium_world_pending();i++)geometrium_world_update(x,z);CHECK(!geometrium_world_pending());
}
static void test_persistence(const char *directory) {
    ...
    for(int i=0;i<600;i++)CHECK(geometrium_world_set(i*2-600,100,i%23-11,(i%3)+BLOCK_DIRT));
    CHECK(geometrium_edits_count()==600 && geometrium_edits_dirty());CHECK(geometrium_edits_save());CHECK(!geometrium_edits_dirty());
    warm(2048,-2048);warm(-4096,4096);warm(8.5f,8.5f);
    for(int i=0;i<600;i++)CHECK(geometrium_world_cell(i*2-600,100,i%23-11)==(i%3)+BLOCK_DIRT);
    geometrium_world_build(GEOMETRIUM_WORLD_SEED);CHECK(geometrium_edits_count()==600);
    for(int i=0;i<600;i++)CHECK(geometrium_world_cell(i*2-600,100,i%23-11)==(i%3)+BLOCK_DIRT);
    geometrium_world_build(GEOMETRIUM_WORLD_SEED+1);CHECK(geometrium_edits_count()==0);
    geometrium_world_build(GEOMETRIUM_WORLD_SEED);CHECK(geometrium_edits_count()==600);
    FILE *f=fopen(path,"r+b");CHECK(f);CHECK(!fseek(f,24,SEEK_SET));fputc(0x7f,f);fclose(f);
    geometrium_world_build(GEOMETRIUM_WORLD_SEED);CHECK(geometrium_edits_count()==0); /* checksum rejects corruption */
    f=fopen(path,"wb");CHECK(f);fwrite("EJVOX01",1,7,f);fclose(f);
    geometrium_world_build(GEOMETRIUM_WORLD_SEED);CHECK(geometrium_edits_count()==0);
...
static void test_save_migration(const char *directory) {
    ...
    CHECK(geometrium_edits_count()==1 && geometrium_water_level(-2,80,0)==0 && geometrium_world_cell(-1,80,0)==BLOCK_STONE);
    CHECK(geometrium_world_set(-1,80,0,BLOCK_DIRT));CHECK(geometrium_edits_save());
    ...
    geometrium_world_build(GEOMETRIUM_WORLD_SEED);CHECK(!geometrium_edits_count());
