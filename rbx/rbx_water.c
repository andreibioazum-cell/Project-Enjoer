...
void rbx_water_wake(int x,int y,int z) {
    int above=rbx_world_cell(x,y+1,z)==BLOCK_WATER;
    int wet=above || rbx_world_cell(x,y,z)==BLOCK_WATER;
    for (int i=0;!wet && i<6;i++) wet=rbx_world_cell(x+dirs[i][0],y+dirs[i][1],z+dirs[i][2])==BLOCK_WATER;
    if (!wet) return;
    enqueue(x,y,z);
...
static void activate_edit(const RbxEdit *e) {
    /* Also wakes dry edited holes: an adjacent original lake must refill them. */
    for (int i=0;i<8;i++) {
        int x=e->x*2+(i&1),y=e->y*2+((i>>1)&1),z=e->z*2+(i>>2);
        if (rbx_world_active(x,z)) rbx_water_wake(x,y,z);
    }
}
void rbx_water_activate(int cx,int cz) {
    for (const RbxEdit *e=rbx_edit_first(cx,cz);e;e=rbx_edit_next(e)) activate_edit(e);
}
static int supported(int x,int y,int z) {
    int b=rbx_world_cell(x,y-1,z);
    return (b!=BLOCK_AIR && b!=BLOCK_WATER) || (b==BLOCK_WATER && rbx_water_level(x,y-1,z)==0);
}
static int desired(int x,int y,int z,int old) {
    if (old==0) return 0; /* only genuine sources are persistent */
    if (rbx_world_cell(x,y+1,z)==BLOCK_WATER) return RBX_WATER_FALLING;
    int best=RBX_WATER_REACH+1;
    for (int i=1;i<=4;i++) {
        int nx=x+dirs[i][0],nz=z+dirs[i][2];
        int level=rbx_water_level(nx,y,nz);
        if (level<0 || !supported(nx,y,nz)) continue;
        if (level==RBX_WATER_FALLING) level=0; /* a waterfall spreads at its foot */
        if (level+1<best) best=level+1;
    }
    return best<=RBX_WATER_REACH ? best : -1;
}
void rbx_water_update(float d) {
    last_work=0;
    ...
    if (recovery>=0) {
        for (int i=0;i<RECOVERY_BUDGET;i++) {
            const RbxEdit *e=rbx_edit_at(recovery++);
    ...
    for (int i=0;i<jobs;i++) {
        ...
        if (!rbx_world_active(p.x,p.z) || rbx_cell_solid(p.x,p.y,p.z)) continue;
        int old=rbx_water_level(p.x,p.y,p.z),next=desired(p.x,p.y,p.z,old);
        if (old!=next) changes[n++]=(Change){p.x,p.y,p.z,next};
    }
    for (int i=0;i<n;i++) {
        Change p=changes[i];
        rbx_world_water_set(p.x,p.y,p.z,p.level);
    }
}
int rbx_water_pending(void) {return count+(overflow || recovery>=0);}
int rbx_water_last_work(void) {return last_work;}
