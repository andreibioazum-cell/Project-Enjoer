...
void rbx_light_bake(RbxChunk *c) {
    ...
    for (int cz=c->cz-1;cz<=c->cz+1;cz++) for (int cx=c->cx-1;cx<=c->cx+1;cx++) {
        const unsigned char *blocks=cx==c->cx && cz==c->cz ? c->blocks : rbx_world_chunk_blocks(cx,cz);
        if (!blocks) {rbx_terrain_chunk(cx,cz,fallback);blocks=fallback;}
        ...
        for (const RbxEdit *e=rbx_edit_first(cx,cz);e;e=rbx_edit_next(e)) {
    ...
    if (need>c->light_capacity) {
        ...
        if (!p) {app_fail("Недостаточно памяти для освещения");return;}
    ...
    for (int y=bottom;y<height;y++) for (int z=0;z<CACHE_SIDE_LIGHT;z++) for (int x=0;x<CACHE_SIDE_LIGHT;x++) {
        ...
        c->light[(y-bottom)*CACHE_PLANE_LIGHT+z*CACHE_SIDE_LIGHT+x]=(unsigned char)((value&SOLID)?RBX_LIGHT_OPAQUE:value&15);
    }
    c->light_y=bottom;c->light_height=height;c->light_valid=1;
}

int rbx_light_cell(const RbxChunk *c,int x,int y,int z) {
    if (y<c->light_y) return RBX_LIGHT_OPAQUE;
    if (y>=c->light_height) return RBX_LIGHT_MAX;
...
void rbx_light_face(const RbxChunk *c,const int origin[3],int u,int v,int face,unsigned char light[4]) {
    ...
    for (int i=0;i<4;i++) {
        ...
        for (int dv=-1;dv<=0;dv++) for (int du=-1;du<=0;du++) {
            ...
            int l=rbx_light_cell(c,s[0],s[1],s[2]);
            if (l!=RBX_LIGHT_OPAQUE) {sum+=l;count++;}
        }
